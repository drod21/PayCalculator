...
public class TimeRecDataGetter {
     ...
     */
    public void getData() {
        Intent intent = new Intent("com.drod2169.paycalculator.GET_INFO");
        intent.setPackage(getMatchingTimeRecPackage(context, "com.drod2169.paycalculator"));
        intent.putExtra("com.drod2169.paycalculator.FLAGS", 1); // 1=resolve weekly delta
        BroadcastReceiver resultReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent resultIntent) {
                Bundle extras = this.getResultExtras(true);
                Bundle result = extras.getBundle("com.drod2169.paycalculator.RESULT");
                debugBundle(result);
                getFields(result);
            }
        };
